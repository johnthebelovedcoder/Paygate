...
const ContentAnalytics: React.FC<ContentAnalyticsProps> = ({ contentItems }) => {
  ...
  useEffect(() => {
    if (contentItems && contentItems.length > 0) {
      ...
      const totalDownloads = contentItems.reduce((sum, content) => {
        // In a real app, this would come from actual analytics data
        // For now, we'll simulate based on content properties
        const simulatedDownloads =
          content && content.status === 'published' ? Math.floor(Math.random() * 100) + 10 : 0;
        return sum + simulatedDownloads;
      }, 0);

      // Popular content (top 5 by simulated downloads)
      const popularContent = [...contentItems]
        .filter(content => content && content.status === 'published')
        .sort((_a, _b) => {
          const aDownloads = Math.floor(Math.random() * 100) + 10;
          const bDownloads = Math.floor(Math.random() * 100) + 10;
      ...
      // Download trends (simulated data for the past 30 days)
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        // Simulate downloads for this date
      ...
      // Top performers (based on downloads and engagement)
      const topPerformers = [...contentItems]
        .filter(content => content && content.status === 'published')
        .sort((_a, _b) => {
          // Simulate performance score based on multiple factors
          const aScore = Math.floor(Math.random() * 100);
          const bScore = Math.floor(Math.random() * 100);
