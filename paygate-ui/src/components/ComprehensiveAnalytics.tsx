  ...
  useEffect(() => {
    const fetchAdditionalAnalytics = async () => {
      try {
        ...
        const geographicData = await apiService.get('/analytics/geography');
        if (geographicData.success) {
          setGeographicData(geographicData.data);
        }

        // Fetch traffic sources
        const trafficSourcesData = await apiService.get('/analytics/traffic');
        if (trafficSourcesData.success) {
          setTrafficSources(trafficSourcesData.data);
        }

        // Fetch customer lifetime values
        const clvData = await apiService.get('/analytics/clv');
        if (clvData.success) {
          setCustomerLifetimeValues(clvData.data);
        }

        // Fetch revenue forecast
        const forecastData = await apiService.get('/analytics/predictive/revenue-forecast');
