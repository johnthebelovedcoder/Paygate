...
-- 4. Fix some storage utility functions (safe to modify)
...
-- 5. Fix some realtime utility functions (safe to modify)
...
1. These functions are relatively safe to modify as they are basic utility functions
...
-- Verification query to confirm the applied functions now have search_path settings
SELECT 
...
WHERE n.nspname IN ('public', 'auth', 'storage', 'realtime', 'graphql')
AND p.prokind = 'f'
