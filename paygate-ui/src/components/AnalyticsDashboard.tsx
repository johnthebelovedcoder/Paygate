...
const AnalyticsDashboard: React.FC = () => {
  ...
  const [recentPaymentsData, setRecentPaymentsData] = useState<Payment[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const revenueData = await analyticsService.getRevenueData(timeRange);
        setRevenueData(revenueData as RevenueData[]);

        // Fetch top paywalls
        const topPaywalls = await analyticsService.getTopPaywalls();
        setPaywallData(topPaywalls.map(pw => ({
          id: pw.id,
          title: pw.title,
          revenue: pw.revenue,
          conversions: pw.conversions,
          conversionRate: pw.conversionRate
        })));

        // For traffic and conversion data, we'll use mock data since there are no endpoints yet
        ...
        // For recent payments, we'll use mock data for now
        ...
        setRecentPaymentsData(mockRecentPayments);
  ...
  // Use stats from the API instead of calculating from mock data
