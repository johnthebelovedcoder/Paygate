...
const ComprehensiveDashboard: React.FC = () => {
  ...
  const formatCurrency = (value: number): string => {
    // Use USD as default currency if no user is authenticated
  ...
  useEffect(() => {
    if (customerData && customerData.customerGrowth) {
      ...
      if (customerGrowth.length >= 2) {
        const currentMonth = customerGrowth[customerGrowth.length - 1].newCustomers;
        const previousMonth = customerGrowth[customerGrowth.length - 2].newCustomers;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        <Header
          title="Dashboard"
          subtitle="Monitor analytics and performance"
